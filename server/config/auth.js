...
const generatejwt = (ID, type) => jwt.sign({ "id": ID, "type": type }, process.env.APP_SECRET, { expiresIn: "1hr" });

const verifyjwt = (auth) => {
    const result = jwt.verify(auth, process.env.APP_SECRET, (error, decoded) => { if(error) return new GraphQLError(error); return decoded;});
    return result;
};

const generateNewJwt = (refresh) => { return new Promise((resolve, reject) => 
    redisClient.hget(refresh, "token", (error, result) => {
        if(error) reject(new GraphQLError(error));
        const decoded = jwt.decode(result);
        const newjwt = generatejwt(decoded.id, decoded.type, (error, result) => {if(error) return new GraphQLError(error); return result;});
...
const login = (email, password) => {
	...
	const checkMentor = (email, password) => { return dbQuery("SELECT mentor_id, mentor_password FROM Mentors WHERE email = (?) AND absolute_year = (?)",[email, year])
		...
		{ const jwt = generatejwt(data.mentor_id, "mentor"); const refresh = uuid.v4(); redisSet(jwt, refresh); return { "auth": jwt, "refresh": refresh }; }
	...
	const checkOrgAdmin = (email, password) => { return dbQuery("SELECT org_admin_id, org_admin_password FROM Org_Admins WHERE email = (?) AND absolute_year = (?)",[email, year])
		...
		{ const jwt = generatejwt(data.org_admin_id, "orgAdmin"); const refresh = uuid.v4(); redisSet(jwt, refresh); return { "auth": jwt, "refresh": refresh }; }
	...
	const checkSuperAdmin = (email, password) => { return dbQuery("SELECT super_admin_id, super_admin_password FROM Super_Admins WHERE email = (?) AND absolute_year = (?)",[email, year])
		...
		{ const jwt = generatejwt(data.super_admin_id, "superAdmin"); const refresh = uuid.v4(); redisSet(jwt, refresh); return { "auth": jwt, "refresh": refresh }; } 
	...
	const checkApplicant = (email, password) => { return dbQuery("SELECT applicant_id, applicant_password FROM Applicants WHERE email = (?) AND absolute_year = (?)",[email, year])
		...
		{ const jwt = generatejwt(data.applicant_id, "applicant"); const refresh = uuid.v4(); redisSet(jwt, refresh); return { "auth": jwt, "refresh": refresh }; } 
...
module.exports = { login, signUp, hash, compare, generatejwt, verifyjwt, generateNewJwt };
