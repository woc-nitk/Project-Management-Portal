...
var ApplicantResolvers = {
  id: (parent) =>
    dbQuery("SELECT applicant_id FROM Applicants WHERE applicant_id = (?)", [
      parent.applicant_id,
    ]).then((data) =>
      data ? data.applicant_id : new GraphQLError("No such entry")
    ),
  email: (parent) => {
    return dbQuery("SELECT email FROM Applicants WHERE applicant_id = (?)", [
      parent.applicant_id,
  ...
  applicant_year: (parent) =>
    dbQuery("SELECT applicant_year FROM Applicants WHERE applicant_id = (?)", [
      parent.applicant_id,
    ]).then((data) =>
      data ? data.applicant_year : new GraphQLError("No such entry")
    ),
  absolute_year: (parent) =>
    dbQuery("SELECT absolute_year FROM Applicants WHERE applicant_id = (?)", [
      parent.applicant_id,
    ]).then((data) =>
